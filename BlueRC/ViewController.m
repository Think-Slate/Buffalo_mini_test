...
-(void)bleDidReceiveData:(unsigned int *)data length:(int)length{
    ...
    if(data[0] == 0x14B766FF){
        UIAlertView * alert = [[UIAlertView alloc] initWithTitle:@"Success!" message:@"We received the message: 0X14B766FF, we would use that to either ping a server or another phone via bluetooth" delegate:self cancelButtonTitle:@"Close" otherButtonTitles:nil];
